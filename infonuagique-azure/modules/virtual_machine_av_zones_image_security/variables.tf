...
#  description = "The Zone disks in which this Virtual Machine should be created. Conflicts with availability set and shouldn't use both"
#  type        = list(string)
#  default     = []
#}
...
variable "windows_distribution_list_plan" {
  description = "Pre-defined Azure Windows VM plan list"
  type = map(object({
    name          = string
    publisher     = string
    product       = string
...
variable "windows_distribution_plan" {
  default     = "windows2012r2dc_plan"
  description = "Variable to pick an OS flavour for Windows based VM. Possible values include: winserver, wincore, winsql"
}
