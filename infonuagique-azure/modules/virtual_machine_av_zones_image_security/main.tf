...
resource "azurerm_network_security_rule" "nsg_rule" {
  ...
  destination_address_prefix  = var.destination_address_prefix #element(concat(data.azurerm_subnet.snet.address_prefixes, [""]), 0)
...
resource "azurerm_linux_virtual_machine" "linux_vm" {
  ...
  admin_password                  = var.admin_password #var.disable_password_authentication == false && var.admin_password == null ? element(concat(random_password.passwd.*.result, [""]), 0) : var.admin_password
  ...
  dynamic "boot_diagnostics" {
    ...
    content {
      storage_account_uri = var.storage_account_uri #var.storage_account_name != null ? azurerm_storage_account.storeacc.0.primary_blob_endpoint : var.storage_account_uri
    }
  }
...
resource "azurerm_windows_virtual_machine" "win_vm" {
  ...
  name                         = var.virtual_machine_name #var.instances_count == 1 ? substr(var.virtual_machine_name, 0, 15) : substr(format("%s%s", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15)
  computer_name                = var.virtual_machine_name #var.instances_count == 1 ? substr(var.virtual_machine_name, 0, 15) : substr(format("%s%s", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15)
  resource_group_name          = var.resource_group_name
  location                     = var.location
  size                         = var.virtual_machine_size
  admin_username               = var.instances_count == 1 ? substr(var.admin_username, 0, 15) : substr(format("%s%s", lower(replace(var.admin_username, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15)    #var.admin_username
  admin_password               = var.admin_password #== null ? element(concat(random_password.passwd.*.result, [""]), 0) : var.admin_password
  network_interface_ids        = [element(concat(azurerm_network_interface.nic.*.id, [""]), count.index)]
  source_image_id              = var.source_image_id != null ? var.source_image_id : null
  #source_plan_id              =  var.source_plan_id != null ? var.source_plan_id : null
  provision_vm_agent           = true
  allow_extension_operations   = true
  dedicated_host_id            = var.dedicated_host_id
  custom_data                  = var.custom_data != null ? var.custom_data : null
  ...
  dynamic "plan" {
    for_each = var.source_image_id != null ? [] : [1]
    content {
      name = var.custom_image != null ? var.custom_image["name"] : var.windows_distribution_list_plan[lower(var.windows_distribution_plan)]["name"]
      publisher = var.custom_image != null ? var.custom_image["publisher"] : var.windows_distribution_list_plan[lower(var.windows_distribution_plan)]["publisher"]
      product   = var.custom_image != null ? var.custom_image["product"] : var.windows_distribution_list_plan[lower(var.windows_distribution_plan)]["product"]
      }
  }
  os_disk {
    storage_account_type      = var.os_disk_storage_account_type
...
resource "azurerm_managed_disk" "data_disk" {
  ...
  disk_encryption_set_id = var.disk_encryption_set_id
  zones                = each.value.data_disk.zones
  tags                 = var.tags
