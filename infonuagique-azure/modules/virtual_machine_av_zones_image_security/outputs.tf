...
output "windows_network_interface_id" {
  description = "network_interface's for the all windows Virtual Machines"
  value       = var.os_flavor == "windows" ? concat(azurerm_network_interface.nic.*.id, [""]): null
}

output "linux_network_interface_id" {
  description = "network_interface's  for the all linux Virtual Machines"
  value       = var.os_flavor == "linux" ? concat(azurerm_network_interface.nic.*.id, [""]): null
}

output "windows_name" {
  description = "Name of the windows Virtual Machines"
